...
public class SoulFireArmorEntity extends FireArmorEntity {
		...
		for (int i2 = 0; i2 < 2; i2++) {
			...
			for (Entity entity : this.pickEntitys(12, new Vec3(px, py, pz), 1F)) {
				if (entity != this && (this.getFirstPassenger() == null || this.getFirstPassenger() != null && entity != this.getFirstPassenger()) && !this.isAlliedTo(entity) && (entity.isAttackable())) {
					...
					entity.hurt(this.damageSources().source(ModDamageSource.SOUL_FIRE, this.getFirstPassenger() instanceof LivingEntity ? (LivingEntity) this.getFirstPassenger() : this), 5.0F);
				}
			}
		}
	...
	public static AttributeSupplier.Builder createAttributes() {
		return Mob.createMobAttributes().add(Attributes.MAX_HEALTH, 300.0D).add(ForgeMod.ENTITY_GRAVITY.get(), 0.10F).add(Attributes.ARMOR, 17.0F).add(Attributes.ARMOR_TOUGHNESS, 5.0F).add(Attributes.MOVEMENT_SPEED, 0.2D).add(Attributes.KNOCKBACK_RESISTANCE, 1.0D).add(Attributes.ATTACK_DAMAGE, 10.0D).add(Attributes.ATTACK_KNOCKBACK, 2.0D);
	}
